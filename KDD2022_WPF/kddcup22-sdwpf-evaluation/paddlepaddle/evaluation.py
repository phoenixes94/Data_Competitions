...
def evaluate(path_to_src_dir):
    ...
    for i in range(NUM_MAX_RUNS):
        ...
        gt_ys = gt_res["ground_truth_y"]  # (134, 288, 1)
        gt_turbines = gt_res["ground_truth_df"]  # (134, 288, 13)
...
# Folder for offline test
def eval_local(submit_file):
    """
    Desc:
        The interface for the system call
    Args:
    ...
    now_abs_dir = os.path.normpath(os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                                "tests", 
                                                submit_file))
    return evaluate(now_abs_dir)
