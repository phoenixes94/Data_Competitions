...
def load_data(index):
    ...
    with open(os.path.join(now_abs_dir, "data_scale.pkl"), "rb") as g:
        data_scale = pickle.load(g)

    with open(os.path.join(now_abs_dir, "edges.pkl"), "rb") as g:
        edges = pickle.load(g)
...
def predict(settings, index):  # , valid_data, test_data):
    ...
    _data_mean, _data_scale, edges = load_data(index)
    ...
    global_step = load_model( settings["checkpoints"], model)
    model.eval()
    ...
    test_y = paddle.ones(
        shape=[1, 134, settings["output_len"], 12], dtype="float32")
