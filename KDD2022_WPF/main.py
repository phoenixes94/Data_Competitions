...
def seed(seed=42000):
    ...
    paddle.seed(seed)
    paddle.set_flags({'FLAGS_cudnn_deterministic': seed})

    # torch.manual_seed(config['seed']) # 是否有对应的paddle操作
    # torch.cuda.manual_seed(config['seed']) # 是否有对应的paddle操作
    # torch.cuda.manual_seed_all(config['seed']) # 是否有对应的paddle操作
    # torch.backends.cudnn.benchmark = False # 是否有对应的paddle操作
    # torch.backends.cudnn.deterministic = True # 是否有对应的paddle操作
